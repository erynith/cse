...
class DebridStream(private val sharedPref: SharedPreferences) : TmdbProvider() {
    ...
    override suspend fun loadLinks(
        ...
        val service = sharedPref.getString("debrid_service", null)
        val key = sharedPref.getString("debrid_key", null)

        if (res.imdbId.isNullOrBlank() || service.isNullOrBlank() || key.isNullOrBlank()) {
            return false
        }
        ...
        runCatching {
            ...
            res?.streams?.forEach { stream ->
                if (stream.url.contains("/resolve/")) {
                    stream.infohash = stream.url.split("/")[6]
                }
